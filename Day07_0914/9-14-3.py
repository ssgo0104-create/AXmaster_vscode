...
# -------------------------------------------------------------
# 3. 환경 변수 및 Streamlit Secrets 호환 로드
# -------------------------------------------------------------
load_dotenv()
...
def get_secret_or_env(key_name):
    ...
    return (os.getenv(key_name) or "").strip().strip('"').strip("'")

kakao_key = get_secret_or_env("MY_KAKAO_API_KEY")
exchange_key = get_secret_or_env("EXCHANGE_API_KEY")
weather_key = get_secret_or_env("OPENWEATHER_API_KEY")
